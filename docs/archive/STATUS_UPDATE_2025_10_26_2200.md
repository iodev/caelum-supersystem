...
- **Access**: localhost:27017 (from WSL)
...
- **Config Updated**: PIM now uses localhost instead of 10.32.3.27
...
- MongoDB accessible via `localhost` from WSL (not 10.32.3.27)
- **Solution**: Updated all configs to use `localhost`
...
4. ✅ Fixed all localhost vs 10.32.3.27 config issues
...
### Check MongoDB
```bash
timeout 2 bash -c "echo > /dev/tcp/localhost/27017" && echo "Running" || echo "Stopped"
